...
import { MagicEraser } from "@/components/tool/MagicEraser";
...
export default function ToolDetailPage({ params }: { params: Promise<{ category: string, toolId: string }> }) {
  const { category: categoryId, toolId } = use(params);
  ...
  useEffect(() => {
    setIsClient(true);
    // Check if favorited from localStorage (simple mock for now)
    const favorites = JSON.parse(localStorage.getItem('lumora-favorites') || '[]');
  ...
  const handleFavorite = () => {
    ...
    localStorage.setItem('lumora-favorites', JSON.stringify(newFavorites));
    setIsFavorited(!isFavorited);
  };

  const Icon = tool ? ICON_MAP[tool.icon] : Sparkles;
  ...
  useEffect(() => {
    if (result && tool && !tool.requiresFileUpload) {
       ...
       {tool.id !== 'image-eraser' && (
         <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 pb-12 border-b border-zinc-800/50">
            ...
            <div className="flex items-center gap-6">
               ...
               <div className="space-y-2 text-left">
                  <div className="flex items-center gap-3">
                    <h1 className="text-4xl md:text-6xl font-black tracking-tighter text-white uppercase italic px-4 -mx-4">
                      {tool.name}
                    </h1>
                    {tool.pro && (
          ...
          <div className="flex items-center gap-3 relative">
             ...
             >
                <Settings2 size={24} className={cn("relative z-10", isFavorited && "fill-accent-purple")} />
             </button>
          </div>
       </div>
       )}
