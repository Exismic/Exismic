with open("src/components/tool/Dashboard.tsx", "r", encoding="utf-8") as f:
    content = f.read()

if "CATEGORY_ANIM_STYLES" not in content:
...
old_creative = """const CREATIVE_SUITE: DashboardAction[] = [
  {
...
new_creative = """const CREATIVE_SUITE: DashboardAction[] = [
  {
    label: "AI Image Generator",
    description: "Generate stunning high-fidelity 4K art and photos from text prompts.",
    href: "/tools/ai/img-gen",
...
old_dev = """const DEVELOPER_SUITE: DashboardAction[] = [
  {
    label: "Code Studio",
...
new_dev = """const DEVELOPER_SUITE: DashboardAction[] = [
  {
    label: "Code Studio",
    description: "Full stack AI IDE with Monaco editor, live previews, and agentic assistant.",
    href: "/tools/ai/code",
...
old_prod = """const PRODUCTIVITY_SUITE: DashboardAction[] = [
  {
...
new_prod = """const PRODUCTIVITY_SUITE: DashboardAction[] = [
  {
    label: "Invoice Generator",
    description: "Generate sleek professional custom PDF invoices for clients instantly.",
    href: "/tools/productivity/invoice",
...
old_suite_card = re.search(r"function SuiteCard\(\{ action, i \}: \{ action: DashboardAction; i: number \}\) \{.*?\n\}\n", content, re.DOTALL).group(0)

new_suite_card = """function SuiteCard({ action, i }: { action: DashboardAction; i: number }) {
...
content = content.replace(old_suite_card, new_suite_card)

with open("src/components/tool/Dashboard.tsx", "w", encoding="utf-8") as f:
    f.write(content)
